...
COMPOSITION_RANGES = {
    "CaO": (1.66, 1.66, 0.0),
    "SiO2": (80.94, 80.94, 0.0),
    "Al2O3": (10.64, 10.64, 0.0),
    "Fe2O3": (2.29, 2.29, 0.0),
    "FeO": (0.0, 0.0, 0.0),
    "MgO": (0.19, 0.19, 0.0),
}
...
# has been bracketed by the coarse scan. This improves numerical location of
# T01 through T99 without making every scan interval extremely fine.
...
def equilibrium_state(dbf, components, phases, component_composition, temperature_c):
    ...
    phase_names = np.asarray(result.Phase.values).ravel()
    phase_amounts = np.asarray(result.NP.values, dtype=float).ravel()
    valid = np.isfinite(phase_amounts) & (phase_names != "")
    stable_phases = [
        (str(name), float(amount))
        for name, amount in zip(phase_names[valid], phase_amounts[valid])
    ]
...
def summary_for_composition(composition_id, oxide_composition, dbf, components, phases):
    ...
    milestone_temperatures = {}
    for milestone in LIQUID_FRACTION_MILESTONES:
        temperature_c, _, _ = crossing(milestone)
        milestone_temperatures[f"T{int(round(milestone * 100)):02d}_degC"] = temperature_c

    true_liquidus_temperature, _, true_liquidus_phases = crossing(1.0 - PHASE_FRACTION_TOLERANCE)
    max_scan_point = max(scan_rows, key=lambda row: row[1])
    ...
    summary = {
        ...
        **milestone_temperatures,
        "practical_liquidus_degC": milestone_temperatures["T99_degC"],
        "true_equilibrium_liquidus_degC": true_liquidus_temperature,
        "scan_max_liquid_fraction_NP": max_scan_point[1],
...
def main():
    ...
    for composition_id, composition in enumerate(compositions, start=1):
        ...
        try:
            summary, scan_records = summary_for_composition(
                composition_id, composition, dbf, components, phases
            )
